...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(GROQ_URL, {
      ...
      body: JSON.stringify({ model: 'llama-3.1-8b-instant', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: 'Analiza esto:\n\n' + content }], temperature: 0.3, max_tokens: 1200 }),
    })
