...
export default function ProjectsClient({ initialProjects, userId }: Props) {
  ...
  const handleUseTemplate = (template: typeof TEMPLATES[0]) => {
    ...
    setTimeout(() => {
      window.__templateData = template
    }, 50)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => { setEditingProject(null); setShowModal(true) }} className="btn-primary text-sm">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {TEMPLATES.map((t, i) => (
              <button key={i} onClick={() => handleUseTemplate(t)}
                className="text-left p-4 rounded-xl border-2 border-dashed border-gray-200 hover:border-sky-400 hover:bg-sky-50 transition-all group">
