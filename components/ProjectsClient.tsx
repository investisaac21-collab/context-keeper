...
export default function ProjectsClient({ initialProjects, initialVariables, userId, userEmail, plan = 'free' }: Props) {
  ...
  async function handleSave(data: Partial<Project>) {
    ...
    if (editingProject) {
      ...
      if (created) {
        ...
        showToast('ÂÃÂ¡Proyecto creado!')
      }
    }
    setLoading(false)
    setShowModal(false)
  ...
  async function handleDelete(id: string) {
    if (!confirm('ÂÃÂ¿Seguro que quieres eliminar este proyecto? Esta acciÃ³n no se puede deshacer.')) return
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(project => (
            <ProjectCard
              ...
              plan={plan}
              onCopy={() => { setHasCopied(true); showToast('ÂÃÂ¡Prompt copiado!') }}
