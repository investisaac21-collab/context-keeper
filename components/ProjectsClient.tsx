...
export default function ProjectsClient({ initialProjects, initialVariables, userId, userEmail, plan = 'free' }: Props) {
  ...
  async function handleSave(data: { name: string; tag: string; context: string }) {
    setLoading(true)
    if (editingProject) {
  ...
  async function handleDuplicate(project: Project) {
    if (isFreeLimitReached) {
      alert('Has alcanzado el lÃÂÃÂ­mite del plan Free. Actualiza a Pro para duplicar proyectos.')
      return
    }
    const { data: created } = await supabase
      .from('projects')
  ...
  const premiumFeatures = [
    ...
    { icon: '&#128228;', label: 'ExportaciÃÂÃÂ³n avanzada' },
  ]
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      {isFreeLimitReached && (
        <div className="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 p-5 text-white shadow-md">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <p className="font-bold text-base">Has alcanzado el lÃÂÃÂ­mite del plan Free</p>
          ...
          <div className="mt-4">
            ...
            <span className="text-xs text-indigo-200 mt-1.5 block">
              {projects.length}/{FREE_LIMIT} proyectos &#8212; lÃÂÃÂ­mite alcanzado
            </span>
          </div>
        </div>
      )}
      ...
      {projects.length === 0 && !showModal && (
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Elige una plantilla para crear tu primer proyecto rÃÂÃÂ¡pidamente.
          </p>
