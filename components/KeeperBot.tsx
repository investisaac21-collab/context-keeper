...
export default function KeeperBot({ userId, plan }: KeeperBotProps) {
    ...
    supabase.from('projects').select('name, context').eq('user_id', userId).limit(20)
      .then(c => supabase.from('keeper_profiles').select('name, role, tone').eq('user_id', userId).limit(10)
        .then(p => {
          ...
          const msg = n > 0
            ? 'Hola! Tienes ' + n + ' contexto' + (n !== 1 ? 's' : '') + ' y ' + profList.length + ' perfil' + (profList.length !== 1 ? 'es' : '') + '. \u00bfEn qu\u00e9 te ayudo?'
            : 'Hola! Soy Keeper Bot. \u00bfEn qu\u00e9 te puedo ayudar?'
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        className={open ? 'fixed bottom-6 right-6 z-50 w-14 h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all duration-300 bg-zinc-800 border border-zinc-700 rotate-45 scale-95' : 'fixed bottom-6 right-6 z-50 w-14 h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all duration-300 bg-violet-600 hover:bg-violet-500 hover:scale-110'}
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '520px' }}>
          <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-zinc-500">{dataLoaded ? (contexts.length + ' ctx \u00b7 ' + profiles.length + ' perfiles') : 'Cargando...'}</p>
            </div>
            <button onClick={() => { setMessages([]); setDataLoaded(false) }} className="text-zinc-600 hover:text-zinc-400 p-1">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ minHeight: '200px', maxHeight: '340px' }}>
            ...
            {messages.map((m, i) => (
              <div key={i} className={m.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                <div className={m.role === 'user' ? 'max-w-xs rounded-2xl rounded-tr-sm px-3 py-2 text-sm bg-violet-600 text-white' : 'max-w-xs rounded-2xl rounded-tl-sm px-3 py-2 text-sm bg-zinc-800 text-zinc-200'}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          {messages.length <= 1 && (
            <div className="px-4 pb-2 flex gap-2 flex-wrap">
              {['\u00bfQu\u00e9 tengo?', 'Ayuda con marketing', '\u00bfC\u00f3mo uso Keeper?'].map(s => (
          ...
          <div className="px-3 pb-3">
            <div className="flex gap-2 bg-zinc-800 border border-zinc-700 rounded-xl px-3 py-2 focus-within:border-violet-500">
              <input
                ...
                disabled={loading}
              />
              <button onClick={send} disabled={!input.trim() || loading} className="text-violet-400 hover:text-violet-300 disabled:opacity-30">
