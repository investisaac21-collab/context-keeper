...
export default function ProfilesClient({ userId, userEmail, plan, initialProfiles }: Props) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      {toast && (
        <div className={"fixed top-5 right-5 z-[100] px-5 py-3 rounded-xl shadow-lg text-sm font-medium " + (toastType === 'ok' ? 'bg-zinc-900 text-white' : 'bg-red-600 text-white')}>
          {toast}
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-400 text-sm">Identidades persistentes para tu IA. Define tono, rol y reglas. Copia y pega en cualquier chat.</p>
          </div>
          <button
            onClick={openNew}
            className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shadow-none"
          >
        ...
        {/* Stats bar */}
        {profiles.length > 0 && (
          <div className="flex items-center gap-4 mb-6 p-3 bg-zinc-900 rounded-xl border border-zinc-700 text-sm">
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-violet-500"></div>
              <span className="text-zinc-300 font-medium">{profiles.length} perfil{profiles.length !== 1 ? 'es' : ''}</span>
            </div>
            <div className="w-px h-4 bg-zinc-700"></div>
            <span className="text-zinc-500 text-xs">Los perfiles se sincronizan entre dispositivos</span>
        ...
        {profiles.length === 0 && (
          <div className="text-center py-20">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-100 to-purple-100 flex items-center justify-center mx-auto mb-4 shadow-none">
            ...
            <div className="mt-10 max-w-xs mx-auto text-left">
              <p className="text-xs text-zinc-500 text-center mb-3 uppercase tracking-wider">Ejemplo</p>
              <div className="bg-zinc-900 rounded-2xl border border-violet-200 p-4 shadow-none">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    <div className="text-white font-semibold text-xs">Marketing Lead</div>
                    <div className="text-violet-500 text-xs">Keeper Profile</div>
                  </div>
                </div>
                <div className="space-y-1.5">
                  <div className="flex gap-2"><span className="text-zinc-500 text-xs w-8">Rol</span><span className="text-zinc-300 text-xs">CMO experto en B2B SaaS</span></div>
                  <div className="flex gap-2"><span className="text-zinc-500 text-xs w-8">Tono</span><span className="text-zinc-300 text-xs">Persuasivo, directo</span></div>
                </div>
              </div>
            </div>
          </div>
        ...
        {profiles.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {profiles.map((p) => (
              <div key={p.id} className="bg-zinc-900 rounded-2xl border border-zinc-700 overflow-hidden hover:border-violet-300 hover:shadow-md transition-all group">
                <div className="p-5 pb-4">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={"w-10 h-10 rounded-xl bg-gradient-to-br " + getColor(p.name) + " flex items-center justify-center font-bold text-white text-sm shadow-none"}>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(p)} className="w-7 h-7 rounded-lg hover:bg-zinc-800 flex items-center justify-center transition-colors" title="Editar">
                  ...
                  <div className="space-y-2">
                    {p.role && (
                      <div className="flex gap-2 items-start">
                        ...
                        <span className="text-zinc-200 text-xs leading-relaxed">{p.role}</span>
                      </div>
                    )}
                    {p.tone && (
                      <div className="flex gap-2 items-start">
                        <span className="text-zinc-500 text-xs w-8 flex-shrink-0 pt-0.5">Tono</span>
                        <span className="text-zinc-200 text-xs leading-relaxed">{p.tone}</span>
                      </div>
                    )}
                    {(p.rules || []).length > 0 && (
                      <div className="flex gap-2 items-start">
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {(p.rules || []).length > 2 && (
                            <span className="text-xs text-zinc-500">+{(p.rules || []).length - 2} mas</span>
                          )}
                        </div>
                      </div>
                    )}
                ...
                <div className="px-5 py-3 bg-zinc-950 border-t border-zinc-800 flex items-center justify-between">
                  ...
                  >
                    ...
                    {chatProfileId === p.id ? 'Cerrar chat' : 'Chatear'}
                  </button>
                </div>
              </div>
            ))}

            {chatProfileId && profiles.find(p => p.id === chatProfileId) && (
              <div className="mt-4">
                <ProfileChat profile={profiles.find(p => p.id === chatProfileId)!} onClose={() => setChatProfileId(null)} />
              </div>
            )}
          </div>
        )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 rounded-2xl w-full max-w-md shadow-none">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
              ...
              <button onClick={() => setShowModal(false)} className="w-8 h-8 rounded-lg hover:bg-zinc-800 flex items-center justify-center transition-colors">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-zinc-200 mb-1.5">Nombre del perfil *</label>
                <input
                  className="w-full border border-zinc-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-200 mb-1.5">Rol</label>
                <input
                  className="w-full border border-zinc-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-200 mb-1.5">Tono y estilo</label>
                <input
                  className="w-full border border-zinc-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-200 mb-1.5">Reglas de comportamiento <span className="text-zinc-500 font-normal">(una por linea)</span></label>
                <textarea
                  className="w-full border border-zinc-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-200 mb-1.5">Contexto extra <span className="text-zinc-500 font-normal">(opcional)</span></label>
                <textarea
                  className="w-full border border-zinc-700 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent resize-none"
              ...
              {/* Avatar */}
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">Avatar <span className="text-zinc-600 font-normal">(URL imagen o gif)</span></label>
                <input type="url" placeholder="https://..." value={form.avatar_url} onChange={e => setForm(f => ({...f, avatar_url: e.target.value}))} className="w-full px-4 py-2.5 bg-zinc-800 border border-zinc-700 focus:border-violet-500 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none transition-colors" />
                {form.avatar_url && (
                  <div className="mt-2 flex items-center gap-2">
                    <img src={form.avatar_url} alt="preview" className="w-12 h-12 rounded-xl object-cover border border-zinc-700" onError={(e: any) => (e.currentTarget.style.display='none')} />
                    <span className="text-zinc-500 text-xs">Vista previa</span>
                  </div>
                )}
            </div>
            <div className="flex gap-3 px-6 pb-6">
              <button onClick={() => setShowModal(false)} className="flex-1 border border-zinc-700 text-zinc-200 text-sm font-semibold py-2.5 rounded-xl hover:bg-zinc-950 transition-colors">
                Cancelar
              </button>
              <button
                onClick={handleSave}
...
// ProfileChat - mini IA integrada por perfil
    ...
    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800 bg-zinc-800/50">
        {profile.avatar_url ? (
          ...
          <div className="w-8 h-8 rounded-xl bg-violet-700 flex items-center justify-center text-white font-bold text-xs">
            {profile.name[0].toUpperCase()}
          </div>
        )}
        <div>
          <p className="text-sm font-semibold text-white">{profile.name}</p>
          {profile.role && <p className="text-xs text-zinc-400">{profile.role}</p>}
        </div>
      ...
      <div className="border-t border-zinc-800 p-3 flex gap-2">
        <input
          ...
          disabled={loading}
          className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-700 focus:border-violet-500 rounded-xl text-sm text-white placeholder-zinc-600 focus:outline-none transition-colors"
        />
        <button
          onClick={send}
